...
| Run noisy command | `icmg run <cmd>` (Tkil filter — 60-90% smaller; **delta-only default-on**: repeat runs emit only changed lines) |
| Repeat run, see only what changed | `icmg run <cmd>` — automatic on 2nd+ run (delta-only, default-on) |
| Repeat run, full output | `icmg run --no-delta <cmd>` (bypass delta) |
| See previous run snapshot | `icmg run --last-full <cmd>` |
| Output auto-tiered by urgency | `icmg run <cmd>` — automatic: error=full, warning=head+count, success=`.`/`~ ok` (default-on) |
| Force full output (any tier) | `icmg run --no-tier <cmd>` (or ICMG_NO_TIER=1) |
