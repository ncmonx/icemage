...
static int doImport(ContextNodeStore& store, const std::vector<std::string>& args,
    ...
    for (auto& fpath : files) {
        ...
        for (auto& sec : sections) {
            ...
            node.tier        = isHotSection(sec.title) ? "hot" : "cold";
            node.tags        = "[]";
            node.active      = true;
...
static int doSlim(ContextNodeStore& store, const std::vector<std::string>& args) {
    ...
    if (sections.empty() && !fs::exists(fpath)) {
        ...
        out << "> Full content stored in icmg DB. Use `icmg knowledge list` to browse.\n";
        out << "> Hook injection active — relevant sections loaded per-turn automatically.\n\n";
        out << "## Sections\n\n";
        for (auto& n : nodes)
    ...
    std::ostringstream out;
    out << "# Context Graph (managed by icmg)\n\n";
    out << "> Full section content in icmg DB. Run `icmg claudemd import` to update.\n";
    out << "> Use `icmg knowledge list` to browse. Hook injects relevant context per-turn.\n\n";
    for (auto& sec : sections) {
        std::string key = slugify(sec.title);
        out << "## " << sec.title << "\n";
