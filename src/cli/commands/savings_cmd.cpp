...
public:
    ...
    void usage() const override {
        std::cout <<
            ...
            "  --json             Machine output\n"
            "  --model NAME       Default $/MTok rates by model (or set ICMG_MODEL)\n"
            "  --rate-input N     Input $/MTok (override; default = model or 3.0)\n"
            "  --rate-output N    Output $/MTok (override; default = model or 15.0)\n";
    }
    ...
    int run(const std::vector<std::string>& args) override {
        ...
        // Per-model default rates (--model / ICMG_MODEL); explicit --rate-* wins.
        ...
        double rate_in = rates.in, rate_out = rates.out;
...
<h2 style="font-size:18px;color:#58a6ff;margin:32px 0 12px">Daily savings (last )HTML"
        ...
        os << R"HTML(<div class="footer">Generated by `icmg savings --html` · costs estimated at $)HTML"
           << "input + output $/MTok per --model/--rate (default Sonnet 4.5). Lower bound.</div>"
           << "</body></html>";
