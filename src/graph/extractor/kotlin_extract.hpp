#pragma once
// Kotlin import/symbol extraction (regex, js/php/ruby family). Pure + testable.
//   package X                         -> namespaces
//   import X                          -> imports
//   class/object/interface X          -> classes
//   fun name                          -> functions
...
inline ExtractResult extractKotlin(const std::string& content) {
    ExtractResult res;
    ...
    static const std::regex re_type(
        R"(^\s*(?:public\s+|private\s+|internal\s+|protected\s+|abstract\s+|open\s+|sealed\s+|data\s+|enum\s+|final\s+|inner\s+|annotation\s+)*(?:class|object|interface)\s+(\w+))");
    static const std::regex re_fun(
        R"(^\s*(?:public\s+|private\s+|internal\s+|protected\s+|override\s+|open\s+|abstract\s+|suspend\s+|inline\s+|operator\s+|tailrec\s+)*fun\s+(?:<[^>]+>\s+)?(\w+))");
    ...
    while (std::getline(ss, line)) {
        ...
        if (std::regex_search(line, m, re_fun))    res.functions.push_back(m[1].str());
    }
