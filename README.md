...
[![tests](https://img.shields.io/badge/tests-2221%2F2221%20passing-brightgreen)](#)
...
- **v2.13.1** — **Fix: err126 recurrence on Windows Server despite the v2.13.0 Vulkan gate.** The gate checked only that `HKLM\...\Khronos\Vulkan\Drivers` had ≥1 registry value, but a stale GPU driver leaves the value behind after its manifest is gone — false-positive, and `vkCreateInstance` still crashed err126. Two-layer fix: manifest-on-disk verification (`vulkanIcdPresent()` requires ≥1 registered manifest to actually exist, pure unit-tested core) + an error boundary (`llama_backend_init()` wrapped in try/catch, degrading to "LLM unavailable, command continues"). Workaround without upgrade: `ICMG_GGML_NO_VULKAN=1`. **2166/2166 tests (+4 new).**
- **v2.13.0** — **WASM language extractors + host-caps: teach icmg a new language by dropping a `.wasm`, no rebuild.** extractor-v1 ABI (`icmg_alloc`/`icmg_extract`), JSON output maps 1:1 to `ExtractResult`, auto-consulted on graph scan, `priority>0` overrides built-ins. `icmg skill wasm test`. Host-caps: optional `icmg.log(ptr,len)` import via a wasmtime linker, backward-compatible. **2162/2162 ctest (+30 new tests).**
- **v2.12.0** — **Fix: `icmg find` Unicode crash + build.ps1 auto-installs the live binary.** `icmg find` crashed with Windows error 1113 when a scanned path held non-ANSI filenames; three call sites in `find_cmd.cpp` now use `pathU8()` for UTF-8-safe handling (issue #221). `build.ps1` now auto-updates the in-use `~/bin/icmg.exe` after a build via a Windows-safe rename→copy→kill→cleanup sequence. Migration `0046` (delta-output snapshot columns) is now embedded in shipped binaries.
```bash
