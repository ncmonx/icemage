...
## 🛠 v1.20.6 — F7 transparent wrapper peek in bash-rewrite hook

From the v1.20.0 plan (F7). The PreToolUse:Bash rewrite hook matched its regex against the raw tool input, so wrapper shells (`bash -c "grep ..."`, `xargs cargo test`, `npx vitest`, `pnpm exec next build`) silently bypassed the icmg-run redirect — the agent ended up running the noisy underlying command at full output. v1.20.6 peels up to 3 layers of known wrapper prefixes before matching, then suggests the un-wrapped icmg-run form.
...
Three user-reported regressions plus an enhancement:

- **Claude first-launch 60s subprocess-init timeout**: hook entries cold-spawned `icmg.exe` at every fire — saturated Claude's init budget on multi-user shared binary with Defender scan. v1.20.5 reverts to script-based pattern with timeouts + fail-soft tails.
- **`icmg graph rebuild` 7+ min**: wraps full scan in a single `BEGIN TRANSACTION`/`COMMIT`. Minutes → seconds.
- **`icmg --version` from CMD didn't return until keypress**: atexit handler now flushes stdio + injects CR + frees console.
- **jq auto-install**: `icmg init` probes `jq --version`; downloads on Windows or prints install hints on POSIX.
...
`icmg init` hung up to an hour on tiny projects — sync `icmg parallel`, recursive `icacls /T`, unconditional `Add-MpPreference`. v1.20.4 detaches sub-imports, drops `/T`, caches Defender exclusion. Also new: **log-dedup filter (F6)** for `docker logs` / `kubectl logs` / `journalctl`.

ctest 111/111. Drop-in upgrade.

---

> 📜 **Older releases:** see [`CHANGELOG.md`](CHANGELOG.md) for v1.20.3 and earlier.
