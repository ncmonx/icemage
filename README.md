...
- **v1.51.0** - **Manage rules & skills in plain chat**. Tell the AI `hapus rule no-emoji` or `tambah skill clang-fmt format C++ via clang-format` and it routes the request to rule/skill CRUD with fuzzy name resolution (BM25-ish, name-weighted 3x, threshold 0.5). Triggers cover ID+EN: `hapus`/`buang`/`remove`/`delete`/`drop`, `ubah`/`ganti`/`update`/`edit`/`change`, `tambah`/`buat`/`add`/`create`. NL removes use soft-disable (reversible via `icmg rule enable`) - hard delete reserved for `\unrule` slash. New CLI: `icmg skill add/remove/edit` writes `~/.icmg/skills/*.md`. 24+2 new tests green. Opt-out via `ICMG_NO_AUTO_RULE=1`.
