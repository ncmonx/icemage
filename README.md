...
[![tests](https://img.shields.io/badge/tests-1483%2F1483%20passing-brightgreen)](#)
...
- **v2.0.8** - **Ranked persona search and leaner per-turn context.** `icmg profile search "<query>"` now uses an FTS5/bm25 index over the persona store instead of a substring scan, so cross-project profile/skill/note recall returns the most relevant entries first (graceful LIKE fallback when FTS5 is unavailable; the index self-syncs on every write). And the per-prompt recall injector now skips a memory slice whose topic is a near-duplicate of one already surfaced earlier in the same session (a file-backed Jaccard window, reset on each new session), so the same context is not re-spent turn after turn — completing the 7-component injection governor. Plus a Windows test-harness fix (the test entrypoint sets `SetErrorMode`, so subprocess-spawning tests no longer raise a transient drive-not-found dialog). Full automated suite passes (1480 checks).
