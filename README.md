...
- **v2.24.0** — **Landscape pack: grounded memory, honest gaps, real-workload bench, wired compression.** Seven deterministic, zero-LLM features from the 2026-09 AI/AGI landscape scan (agent-memory + context-compression + repo-view + MCP-security literature). **`memory-health --grounding`** — a memory citing a file the code graph no longer knows was probably written before a rename/delete (arXiv 2609.11060); probes our own graph, flags stale-suspects with the vanished refs, never deletes — noise-controlled (import specifiers, TS `js→ts` twins, system headers skipped). **Evicted-vs-missed gap tags** — `memory-health --gaps` now says which remedy fits (arXiv 2609.08279): `[missed]` = never held → store it; `[evicted]` = a soft-deleted memory matches the query (≥2 strong tokens) → `icmg memory restore <id>`. **`bench-recall --replay`** — judges the brain on its REAL workload (arXiv 2609.24971): replays `query_history` against the current store; hit-rate, regressed queries (answered then, silent now = prune/forget damage), and answer token cost; exits 1 on regressions (CI-gateable). **Confidence cue on auto-recall** — every surfaced hit reads `[score|high/med/low]` relative to the injection floor (arXiv 2609.24259: models over- and under-trust injected memory without calibration). **`shrink --kind salience --graph-coherence`** — structure-blind per-line scoring drops the line citing `registry.hpp` while keeping its graph-wired neighbor `dispatcher.cpp`; the coherence pass pulls back dropped lines citing files graph-adjacent to kept citations (arXiv 2608.30811, opt-in, capped). **`graph-skeleton --for <task> --focus`** — restricts the view to the task-seed + 1-hop neighborhood and boosts directly-seeded files, so a sparse task seed is no longer drowned by global hub files (arXiv 2609.16936). **MCP per-tool call policy** — exposure filtering is not access control: `ICMG_MCP_DENY=a,b,c` and `ICMG_MCP_READONLY=1` (denies the 14-tool mutating subset) now hold at `tools/call` time and hide denied schemas from `tools/list`; default-off. Also: the Windows zip pollution root cause fixed at the source (`build.ps1` + `pack-win.ps1` share one 22-DLL whitelist, hard content gate deletes a bad zip before upload). 35 new tests — **2497/2497 tests ✓.**
