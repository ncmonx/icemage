...
- **v1.50.0** - **AI auto-saves rules from natural chat**. Tell local AI \"ingat ya jangan buka file lebih dari 500 baris\" or \"remember: always use icmg run for bash\" in chat — it auto-detects the persistent instruction and saves as a rule. Triggers on prefixes like `ingat ya`, `selalu`, `jangan pernah`, `mulai sekarang`, `remember`, `from now on`, `always`, `never`, `rule:` (both Indonesian + English). Saved rule auto-injects into future chats. Opt-out via `ICMG_NO_AUTO_RULE=1`.
