...
[![tests](https://img.shields.io/badge/tests-1962%2F1962%20passing-brightgreen)](#)
...
## ✨ What's new
- **v2.8.4** - **Context versioning, compact handoff, perplexity salience scorer, and full gitignore-style `.icmgignore`.** `icmg compact` synthesizes a compact session-handoff from memory + `.remember/` into `.icmg/compact-handoff.md` (60–80% tokens saved vs re-reading full history on session resume). `icmg context-commit / context-branch / context-merge` gives Git-style context versioning in `.icmg/ctx-vcs/` — snapshot a reasoning state, switch to a named branch to isolate a reasoning path, then merge two branch snapshots (inspired by a GCC-paper finding that structured context versioning improves SWE-Bench scores 80%+). `icmg shrink --scorer=llama` adds a LLMLingua-style perplexity salience slot (routes to llama backend when available, falls back to heuristic). `.icmgignore` upgraded to full gitignore semantics: `**`, `?`, `!negation`, `dir/*`, last-rule-wins. All new commands documented in the AGENTS.md decision table. Full automated suite passes (1962 checks).

- **v2.8.3** - **Seven token-efficiency wins.** `icmg smart-read <file>` auto-selects the reading strategy by file size: >50KB emits symbols-only (~90% savings), 10-50KB emits first 100 lines, <10KB emits full content. `icmg batch-read <f1> <f2> ...` reads N files in a single tool call with === dividers (saves N-1 round-trips). `icmg context --symbols-only` emits only the symbol list, skipping body and memory (~80% savings on large files). `icmg context --skeleton` emits function/class signatures (first line of each symbol) without bodies. `icmg shrink --aggressive` drops comments, collapses blanks, and abbreviates `std::` tokens. `icmg diff-summary --full --compact` strips context lines, keeping only +/-/@@ headers. All new commands documented in AGENTS.md decision table. 1952 tests pass.

- **v2.8.2** - **Five new commands + smarter find ranking.** `icmg patch` applies unified diff (`-p1`) directly to files. `icmg recall --since 7d/24h/EPOCH` filters memory by age. `icmg memory export` dumps the entire memory store to JSON or Markdown. `icmg find` now blends **PageRank** into its TF-IDF ranking (up to +30% boost for high-authority files). `icmg context <file> --diff` is now in the decision table (saves 60-80% on re-reads). Also includes **v2.8.1**: `icmg fuzzy-edit <file> --old "..." --new "..."` -- three-level whitespace-tolerant edit (exact -> ws-normalised -> anchor-line) that eliminates the old_string not found retry loop from indent drift. Full automated suite passes (1944 tests).
...
_See [`CHANGELOG.md`](CHANGELOG.md) for v2.7.x and earlier._
```bash
