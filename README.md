...
- **v2.0.0** - **Lean & Lossless Compaction governor + a zoned memory platform.** icmg's flagship release. A deterministic **context governor** (`icmg govern`) targets one pain — Claude Code's compaction stalling a session mid-task. Compaction is harness-locked (can't be made async), so the governor makes it **rarer, lossless, and un-surprising** with seven model-free pieces (C1-C7): injection budget, cross-turn dedup, U-shaped ordering (lost-in-the-middle), lossless snapshot/rebuild, idle-compact advisor, tool-output trim, and document intake-trim (`icmg ingest --doc`). Alongside: a **zoned profile/skill store** (`icmg profile`) in a cross-project persona DB, plus **prompt→response history** (`qa-add`/`qa-find`). Also an IDE-hang fix and a `working_set_snapshot` migration (0040). Full automated suite passes (1370 checks).
- **v1.100.0** - **FTS5 search snapshot: code/graph search is now an indexed MATCH, not a full-table scan.** `icmg graph search` / `icmg_code_search` previously did an `O(n)` `LIKE '%q%'` scan of every graph node on each query — slow on large repos. v1.100 adds a `graph_fts` FTS5 index (migration 0039, external-content over `graph_nodes`, kept in sync by triggers) and routes search through `MATCH` + `bm25()` ranking, falling back to `LIKE` automatically if the index is absent. Query input is turned into safe prefix terms (injection-proof). On a ~19K-node graph the snapshot turns a scan into a sub-millisecond lookup. Full automated suite passes (1331 checks).
## 🧭 Where v2 is headed
