...
- **v1.48.0** - **Local AI chat is now a personal assistant**. Conversations persist across sessions (close and reopen — picks up where you left off). Every prompt auto-pulls relevant context from past chats, project memory, your active rules, and available skills. Manage rules and skills from inside chat with new slash commands: `\rule <text>` saves a persistent rule, `\unrule <name>` removes one, `\rules` lists them, `\skill <args>` wraps the `icmg skill` cmd. Plus session controls: `\sessions`, `\resume <id>`, `\new`, `\context off/on`, `\sources`, `\memo <text>`. Bug fixes: long conversations no longer crash (n_batch 16× larger + auto-trim oldest turns), quote chars and special markers safely handled, multi-turn memory works correctly across sessions.
- **v1.47.0** - **Local AI chat that actually remembers + fewer build crashes**. Local AI chat now remembers prior turns in the same session — ask "what did I just say?" and it recalls. Previously every turn started fresh with "halo halo" greeting (the model thought every prompt was the first). Personality stays in character too: no more `[Your name]` template placeholders leaking into replies. Emoji and multi-byte glyphs render correctly on Windows console. New shortcut: prefix any prompt with `!` or `/local ` and that prompt goes to local AI only — cloud API skipped, zero cloud cost. Plus: GPU layer setting now persists in a config file (no need to set the environment variable each shell). And on Windows: rebuilds after small code edits used to fail intermittently with cryptic "Invalid argument" errors — fixed by changing how debug info is stored. Opt-out: `ICMG_NO_LOCAL_ROUTE=1`.
- **v1.46.0** - **Bigger local AI models + 5-10× faster Linux builds**. Local AI registry now offers 7B/8B model entries (Qwen 2.5 7B, Mistral 7B, Llama 3.1 8B) for users with GPU — significantly smarter than the previous 3B class ceiling. Install: `icmg llm install qwen2.5-7b-q4 --yes --skip-verify`. Linux builds get a new optional native-workspace setup: clone the repo to your WSL home (ext4 native) once via `scripts/setup-wsl-native.sh`, and every future build skips the slow filesystem-translation step — ~5-10× faster.
- **v1.45.0** - **Smarter graph cache for faster repeat lookups + roadmap published**. Internal node cache upgraded to true LRU (least-recently-used) with double the capacity — repeated lookups of the same files stay hot in memory longer, ~10-30% faster on repeat-heavy workloads. Zero user-facing API change. Several user-requested features now documented in the planning docs for v1.46+: smart router (auto local-vs-cloud routing per prompt), bigger local AI models (7B/8B registry entries), conversation memory for local AI (separate storage from project memory), and faster Linux builds via native workspace clone. Community feedback welcome.
- **v1.44.0** - **See where you save the most + cleaner diff summaries + safer filters**. New `icmg savings --per-cmd` shows a per-command breakdown so you know which AI tool calls give you the biggest token savings — actionable insight for habit changes. New `icmg diff-summary --compact` ignores whitespace and blank-line changes for cleaner PR reviews (~50% smaller on noisy diffs). Command filtering pipeline now keeps raw output as fallback when something unexpected happens — no more empty results on edge cases. Linux build script also tolerates transient editor-lock sync warnings. Opt-out filter fallback: `ICMG_NO_TEE_FALLBACK=1`.
---
