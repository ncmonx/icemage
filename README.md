...
[![tests](https://img.shields.io/badge/tests-1983%2F1983%20passing-brightgreen)](#)
...
- **v2.9.0** - **Graph-traversal find, graph clustering `--project` mode, and Leiden community detection.** `icmg find --depends-on <file>` walks the code graph forward (BFS) to list every file your target transitively imports; `--used-by <file>` does the reverse — all files that depend on it. Both flags are graph-native (not grep-based), support `--depth N`, and work with the same project DB that powers `icmg graph`. `icmg graph-cluster --project` pulls live graph edges and runs Leiden clustering to group your codebase into communities (`--top-n`, `--json`). Tree-sitter C++ extraction now emits namespace-qualified names and correct `struct` kinds. 5 new TDD tests per feature. Full automated suite passes (1983 checks).
