...
- **v1.55.0** - **Mega bundle: 5 features**. **G7** `icmg-patch` inline file-patch CLI (regex/py/line-replace modes + dry-run + edit-prep auto-allowlist) eliminates temp .py detour. **F11** persona-inject shrink (session-state marker dedup, 90% per-turn save on non-first prompts, 1059B->101B). **Sub-D D3** 4 new tree-sitter AST extractors (Go/Rust/Java/C++) replace regex fallback - captures functions, methods, structs/classes/interfaces/enums, base classes/traits, calls/macro-invocations. **Sub-D D7** Leiden community detection (`icmg graph-cluster --algorithm leiden`) - 3-phase (move/refine/aggregate), resolution-adjusted modularity, edge-list API. **G1** `icmg-advisor` proactive hint hook - UserPromptSubmit emits `[advisor] consider: <next-step>` from 4 bounded heuristics (uncommitted > 4, release-branch ahead > 3, recall hit, recent build errors); auto-created by `icmg init`, opt-out `ICMG_NO_ADVISOR=1`.
