...
- **v1.48.2** - **Personal-assistant chat (persistent + context-aware)**. Conversations persist across sessions — close and reopen, chat continues. Every prompt auto-pulls relevant context from past chats, project memory, your active rules, and available skills. Manage rules and skills from inside chat: `\rule <text>`, `\unrule <name>`, `\rules`, `\skill <args>`. Plus session controls: `\sessions`, `\resume <id>`, `\new`, `\context off/on`, `\sources`, `\memo <text>`. Long conversations no longer crash (prompt chunk-decoded + context window 4× larger). Special characters and multi-turn memory handled correctly. Hotfix v1.48.1 + v1.48.2 addressed crash regressions during initial rollout.
