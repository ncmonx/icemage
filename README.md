...
- **v2.9.0** â€” **Graph-traversal find, graph clustering --project mode, and Leiden community detection.** `icmg find --depends-on <file>` / `--used-by <file>` BFS forward/reverse closure via GraphStore. `icmg graph-cluster --project` pulls live edges and runs Leiden clustering with `--top-n` and `--json`. Tree-sitter C++ extractor with namespace-qualified symbol names. **1983/1983 ctest (+5 new tests).**
- **v2.8.4** â€” **Context versioning, compact handoff, perplexity salience scorer, and full gitignore-style .icmgignore.** `icmg compact` synthesizes a compact session-handoff from memory + `.remember/` (60-80% tokens saved). `icmg context-commit/branch/merge` gives Git-style context versioning in `.icmg/ctx-vcs/`. `icmg shrink --scorer=llama` adds LLMLingua-style perplexity salience. `.icmgignore` upgraded to full gitignore semantics: `**`, `?`, `!negation`, `dir/*`, last-rule-wins. **1962/1962 ctest.**
- **v2.8.3** â€” **Seven token-efficiency commands in one batch â€” read smarter, shrink harder, diff leaner.** `icmg smart-read` auto-selects strategy by size (>50KB=symbols-only, 10-50KB=truncation, <10KB=full). `icmg batch-read` collapses N file reads into 1 tool call. `icmg context --symbols-only` / `--skeleton`. `icmg shrink --aggressive`. `icmg diff-summary --compact`. **1952/1952 ctest.**
```bash
