...
- **v1.49.0** - **Tunable AI memory + auto schema upgrades**. The local AI context window (memory size) is now adjustable without rebuilding — set via env var or a small config file. Drop the value to fit bigger models on graphics cards with limited memory (e.g. 7B-class models on 4 GB cards). Database schema migrations now apply automatically when the binary embeds new versions — no more silent skips. Win-only build this release; Linux returns when CI is automated.
