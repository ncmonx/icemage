...
[![tests](https://img.shields.io/badge/tests-2177%2F2177%20passing-brightgreen)](#)
...
- **v2.13.2** — **err126 root cause fixed: `icmg context` and `icmg graph update` no longer crash on GPU-less Windows Server.** The long-standing err126 ("The specified module could not be found") on headless Server 2019 / Server Core was finally root-caused with a new self-diagnosing throw-site capture. MSVC's `std::chrono::clock_cast<system_clock>(file_clock)` — used in the file-staleness check — converts through `utc_clock`, which makes the STL lazily `LoadLibrary("icu.dll")` for its time-zone database. On Server SKUs without ICU that load fails and throws `std::system_error(126)`, crashing the two commands that touch the stale-check (`context`, `graph update`). It was never SQLCipher crypto or the Vulkan driver — both earlier theories were wrong, and `ntmarta.dll` was a red herring twice; the culprit was our own leap-second-correct stale-check conversion. Fix: a pure FILETIME epoch-delta conversion (`file_time.hpp`) that loads nothing and cannot throw, replacing `clock_cast` at both sites (behavior identical at the 5-second stale tolerance). Two diagnostics shipped alongside: a first-chance **throw-site capture** that prints the throwing module+offset on any uncaught err126 (names the culprit without a debugger or Process Monitor), and a **`doctor` DB write-probe** so `icmg doctor` no longer reports all-clear on a host where reads work but writes crash. Rule going forward: icmg never uses `clock_cast` / `get_tzdb` / `utc_clock` (all ICU-dependent). **2177/2177 tests (+11 new).**
- **v2.13.1** — **Fix: err126 recurrence on Windows Server despite the v2.13.0 Vulkan gate.** The v2.13.0 gate checked only that `HKLM\...\Khronos\Vulkan\Drivers` had ≥1 registry value — but each value's *name* is the path of the driver's JSON manifest, and an uninstalled/stale GPU driver commonly leaves the value behind after the manifest file is gone. On such hosts the gate false-positived, `llama_backend_init()` ran, and `vkCreateInstance` still crashed err126. Two-layer fix: (1) **manifest-on-disk verification** — `vulkanIcdPresent()` now enumerates the registered manifest paths and requires at least one to actually exist on disk (pure decision core, unit-tested: stale-only → absent, mixed live+stale → present); (2) **error boundary** — `llama_backend_init()` wrapped in try/catch so any init failure degrades to "LLM unavailable, command continues" instead of killing the process. Instant host-side workaround without upgrading: `ICMG_GGML_NO_VULKAN=1`. **2166/2166 tests (+4 new).**
- **v2.13.0** — **WASM language extractors + host-caps: teach icmg a new language by dropping a `.wasm`, no rebuild.** Extends the sandboxed WASM skill system (previously filter-only) with a second ABI and a host-call seam. A registered `.wasm`/`.wat` module can now act as an `extractor-v1` — it receives a source file and returns JSON `{context, imports, namespaces, classes, functions, tables}` that maps 1:1 to icmg's `ExtractResult` and feeds straight into the code graph (exports `icmg_alloc`/`icmg_extract`, mirroring the proven `filter-v1` convention; malformed output fails open to "no symbols", never a crash). `graph scan`/`graph update` auto-consult registered extractors — filling a language gap or, with `priority > 0`, overriding a built-in (manifest carries `language`, `extensions`, `priority`, `min_icmg`). New `icmg skill wasm test <name> --file <src>` runs an extractor and prints extracted symbols. Host-caps enabler: modules may optionally import `icmg.log(ptr,len)` via a wasmtime linker — fully backward-compatible (import-less modules instantiate as before; on an older `wasmtime.dll` host-caps silently disables). The seam for future read-only host calls (`icmg.recall`, `icmg.graph_symbol`) — sandbox stays strict. **2162/2162 ctest (+30 new tests).**
