...
- **v1.34.0** - **Anti-amnesia Layer 1 + Layer 2 (7 items, leverages existing data)**: every UserPromptSubmit now also injects (A1) top-2 `errors-resolved%` matching prompt keywords, (A2) top-2 `fail:*` anti-patterns, (A3) last 3 `[saved]` decisions from `session-log.md`, (A4) drift banner when decisions superseded in last 24h. Plus 3 hard PreToolUse gates: (A5/ID=20) rebuild-when-fresh — refuses `cmake --build` when `build/icmg.exe` is newer than every tracked src file (saves 3-10 min per false rebuild), (R2/ID=21+22) version-drift — refuses build when `version.hpp`/`CMakeLists.txt`/`icmg.rc` disagree, (A6/ID=23) ship-phase — refuses bare `gh release create v*` when `.icmg/ship-state.json` is active and not published. Opt-outs: `ICMG_KNOWN_ISSUE_QUIET=1`, `ICMG_FAIL_INJECT_QUIET=1`, `ICMG_DECISIONS_INJECT_QUIET=1`, `ICMG_DRIFT_INJECT_QUIET=1`, `ICMG_REBUILD_FRESH_OFF=1`. Ambient header budget ~540 tok per prompt (R6+R7+A1-A4 combined). Deferred v1.35+: R3 python-block, R4 telemetry, R8 compliance auto-pin.
